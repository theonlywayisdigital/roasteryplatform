...
/* ═══════════════════════════════════════════════════════════════
   ...
   Act 2 (130–300): Orders — order list with status badges
   Act 3 (280–440): CRM — contact card with lifetime value
   Act 4 (420–600): Invoice → Accounting sync

   Transition rule: acts OVERLAP by 20 frames. The outgoing act
   fades/slides out while the incoming act fades/slides in
   simultaneously — no blank gaps between acts.
   ═══════════════════════════════════════════════════════════════ */
...
export const MergedScene4_Operations: React.FC = () => {
  ...
  /* ════════════════════════════════════════════════════════════
     HEADLINES — crossfade with 20-frame overlaps matching acts
     ════════════════════════════════════════════════════════════ */

  const head1Opacity = interpolate(frame, [0, 12, 130, 150], [0, 1, 1, 0], clamp);
  const head1Y = interpolate(frame, [0, 12], [25, 0], easeOut);

  const head2Opacity = interpolate(frame, [130, 150, 280, 300], [0, 1, 1, 0], clamp);
  const head2Y = interpolate(frame, [130, 150], [25, 0], easeOut);

  const head3Opacity = interpolate(frame, [280, 300, 420, 440], [0, 1, 1, 0], clamp);
  const head3Y = interpolate(frame, [280, 300], [25, 0], easeOut);
  ...
  /* ════════════════════════════════════════════════════════════
     ...
     Exit: frames 130–150, shrink + slide left (overlaps Act 2 enter)
     ════════════════════════════════════════════════════════════ */
  ...
  /* ════════════════════════════════════════════════════════════
     ACT 2 — Orders (frames 130–300)
     Enter: frames 130–150, slide in from right (overlaps Act 1 exit)
     Exit: frames 280–300, shrink + slide left (overlaps Act 3 enter)
     ════════════════════════════════════════════════════════════ */
  ...
  // Act 2 enters from the right while Act 1 slides left
  const act2EnterOpacity = interpolate(frame, [130, 150], [0, 1], clamp);
  const act2EnterX = interpolate(frame, [130, 150], [500, 0], easeOut);
  const act2EnterScale = interpolate(frame, [130, 150], [0.85, 1], easeOut);
  ...
  const getFlippingText = (finalText: string, rowIdx: number) => {
    const startFrame = 165 + rowIdx * 15;
  ...
  // Act 2 exit — shrink + slide left (overlaps act 3 enter)
  const act2ExitOpacity = interpolate(frame, [280, 300], [1, 0], clamp);
  const act2ExitScale = interpolate(frame, [280, 300], [1, 0.85], easeOut);
  const act2ExitX = interpolate(frame, [280, 300], [0, -500], easeOut);
  ...
  /* ════════════════════════════════════════════════════════════
     ACT 3 — CRM (frames 280–440)
     Enter: frames 280–300, slide in from right (overlaps Act 2 exit)
     Exit: frames 420–440, shrink + slide left (overlaps Act 4 enter)
     ════════════════════════════════════════════════════════════ */

  // Act 3 enters from the right while Act 2 slides left
  const act3EnterOpacity = interpolate(frame, [280, 300], [0, 1], clamp);
  const act3EnterX = interpolate(frame, [280, 300], [500, 0], easeOut);

  // LTV counter ticks up
  const ltvVal = Math.round(interpolate(frame, [310, 370], [0, 2840], easeOut));

  // Background offset cards
  const bg1Opacity = interpolate(frame, [300, 315], [0, 0.35], clamp);
  const bg2Opacity = interpolate(frame, [303, 318], [0, 0.55], clamp);

  // CRM detail stagger
  const crmFo = (d: number) => interpolate(frame, [d, d + 14], [0, 1], clamp);
  const crmFy = (d: number) => interpolate(frame, [d, d + 14], [14, 0], easeOut);

  // Act 3 exit — shrink + slide left (overlaps act 4 enter)
  const act3ExitOpacity = interpolate(frame, [420, 440], [1, 0], clamp);
  const act3ExitScale = interpolate(frame, [420, 440], [1, 0.85], easeOut);
  const act3ExitX = interpolate(frame, [420, 440], [0, -500], easeOut);
  ...
  /* ════════════════════════════════════════════════════════════
     ...
     Enter: frames 420–440, slide in from right (overlaps Act 3 exit)
     ════════════════════════════════════════════════════════════ */

  // Act 4 enters from the right while Act 3 slides left
  const act4EnterOpacity = interpolate(frame, [420, 440], [0, 1], clamp);
  const act4EnterX = interpolate(frame, [420, 440], [500, 0], easeOut);
  const act4EnterScale = interpolate(frame, [420, 440], [0.85, 1], easeOut);
  ...
  // Brand bar
  const barScale = interpolate(frame, [455, 473], [0, 1], easeOut);

  // Status transitions: Draft → Sent → Paid — crossfade, no instant swap
  const draftOpacity = interpolate(frame, [440, 445, 500, 510], [0, 1, 1, 0], clamp);
  const sentOpacity = interpolate(frame, [500, 510, 540, 550], [0, 1, 1, 0], clamp);
  const paidOpacity = interpolate(frame, [540, 550], [0, 1], clamp);

  // Green pulse on Paid
  const paidPulse = interpolate(frame, [550, 565, 580], [0, 1, 0], clamp);

  // Accounting tiles beam in from invoice at frame 560
  const accounting = [
  ...
  const acctTileDelay = (idx: number) => 560 + idx * 10;
  ...
  /* ════════════════════════════════════════════════════════════
     ...
     Acts overlap by 20 frames at each transition point.
     ════════════════════════════════════════════════════════════ */
            ...
            <div style={{ position: "relative", width: 480, height: 340 }}>
              ...
              <div style={{ position: "relative", zIndex: 10 }}>
                <Card style={{ width: 480 }}>
                  {/* Avatar + name */}
                  <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20, opacity: crmFo(310), transform: `translateY(${crmFy(310)}px)` }}>
                  ...
                  {/* Location */}
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, opacity: crmFo(320), transform: `translateY(${crmFy(320)}px)` }}>
                  ...
                  <div style={{
                    ...
                    marginBottom: 14, opacity: crmFo(330), transform: `translateY(${crmFy(330)}px)`,
                  }}>
                  ...
                  {/* Last order */}
                  <div style={{ display: "flex", alignItems: "center", gap: 10, opacity: crmFo(340), transform: `translateY(${crmFy(340)}px)` }}>
          ...
          <div style={{
            ...
            <Card style={{ width: 440, padding: 0, overflow: "hidden" }}>
              <div style={{ padding: "24px 24px 0" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, opacity: invFo(455) }}>
                ...
                {/* Logo + Invoice # */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, opacity: invFo(465), transform: `translateY(${invFy(465)}px)` }}>
              ...
              <div style={{ padding: "0 24px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderTop: "1px solid #F3F4F6", fontSize: 13, color: BRAND.muted, fontFamily: FONT_FAMILY, opacity: invFo(475) }}>
                  <span>Description</span>
                  <span>Amount</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderTop: "1px solid #F3F4F6", fontSize: 15, fontFamily: FONT_FAMILY, opacity: invFo(485), transform: `translateY(${invFy(485)}px)` }}>
                  <span style={{ color: BRAND.dark }}>House Blend × 10</span>
                  <span style={{ fontWeight: 600, color: BRAND.dark }}>£85.00</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", padding: "14px 0", borderTop: `1px solid ${BRAND.border}`, opacity: invFo(495), transform: `translateY(${invFy(495)}px)` }}>
              ...
              {/* Pay Now button */}
              <div style={{ padding: "8px 24px 24px", display: "flex", justifyContent: "center", opacity: invFo(505) }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14, position: "relative" }}>
              <span style={{ fontSize: 14, fontWeight: 600, color: BRAND.dark, fontFamily: FONT_FAMILY, marginBottom: 4, opacity: interpolate(frame, [555, 565], [0, 1], clamp) }}>Accounting</span>
